...
(`build.ability_order`, from real per-hero analytics — see the generator section above) maps onto N
...
upgrade's index (0→1, 1→2, 2→5 — looked up by position; the third upgrade tier, `◆5`, does appear for
heroes whose real sequence includes it). Columns are 16px wide with a 1px gap (T21) — sized so the
longest real sequence observed across all 38 heroes (15 steps, Infernus's) fits inside the panel at
390px with no horizontal scrollbar; the row content still scrolls horizontally inside the panel
(`overflow-x: auto`) as a fallback for a hypothetical longer sequence or a narrower viewport, in which
case each row's navy background is sized to its own full content width (`width: max-content`, not
stretched to the visible viewport) so it never stops short of the scrolled content. The page itself
never scrolls horizontally, verified in both the jsdom suite and the Playwright mobile spec.
